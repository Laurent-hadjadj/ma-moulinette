...
class ClientService
{
    ...
    private static string $erreur401 = "Erreur d'Authentification. La clé n'est pas correcte  (Erreur 401).";
    private static string $erreur403 = "Vous n’êtes pas autorisé à vous connecter (Erreur 403).";
    private static string $erreur404 = "Le service n'a pas trouvé les éléments (Erreur 404).";
    private static string $erreur407 = "La requête n'a pas été appliquée à cause d'un manque d'authentification (Erreur 407).";
    private static string $erreur414 = "L'URI demandée par le client est plus trop longue (Erreur 414).";
    private static string $erreur418 = "«Je suis une théière », je refuse de préparer du café (Erreur 418).";
    private static string $erreur422 = " (Erreur 422).";
    private static string $erreur429 = "Le client a envoyé trop de requêtes en un temps donné (Erreur 429).";
    private static string $erreur500 = "Le serveur a rencontré un problème inattendu qui l'empêche de répondre à la requête (Erreur 500).";
    private static string $erreur502 = "Le serveur, agissant comme une passerelle ou un proxy, a reçu une réponse invalide (Erreur 502).";
    private static string $erreur504 = "Temps d’attente d’une réponse écoulé... (Erreur 504).";
    private static string $erreur505 = "La version du protocole HTTP utilisée dans la requête n'est pas prise en charge par le serveur (Erreur 505).";
    ...
    private function handleTimeoutException(TimeoutException $e): array {
        $this->logger->error("[handleTimeoutException] ❌ ". self::$erreur504, [
                'code' => 504,
                'erreur' => $e->getMessage() ? $e->getMessage() : null,
            ]);
        return ['code' => 504, 'erreur' => self::$erreur504];
    }
    ...
    public function httpSonarQube(string $url): array
    {
        ...
        try {
            ...
            $url = $response->getInfo('url') . " - " . self::$responseData . substr($responseBody, 0, 100);
        ...
        throw new UnexpectedExecutionPathException(self::$exceptionInattendu . __METHOD__);
    }
    ...
    public function httpActuator(string $url, string $user, string $password): array
    {
        try {
            ...
            $url = $response->getInfo('url') . " - " .
                self::$responseData . substr($responseJson, 0, 100);
