...
class ClientActivite
{
    ...
    {
        if (empty($this->params->get('sonar.activite.token')) && empty($this->params->get('sonar.activite.user'))){
            return ['code'=> 401];
        }

        if (empty($this->params->get('sonar.activite.token'))) {
            $user = $this->params->get('sonar.activite.user');
            $password = $this->params->get('sonar.activite.password');
        } else {
            $user = $this->params->get('sonar.activite.token');
            $password = '';
        }
