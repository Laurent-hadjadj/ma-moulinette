...
class ApiEnregistrementController extends AbstractController
{
    ...
    public function enregistrement(Request $request): response
    {
        ...
        $map=['maven_key'=>$data->maven_key, 'analyse_key'=>$data->analyse_key,
            ...
            'nombre_ligne'=>$data->nombre_ligne, 'nombre_ligne_code'=>$data->nombre_ligne_code, 'couverture'=>$data->couverture, 'duplication_density'=>$data->duplication_density, 'sqale_debt_ratio'=>$data->sqale_debt_ratio, 'tests'=>$data->tests, 'violations'=>$data->violations, 'dette'=>$data->dette,
