    ...
    public function collecte($portefeuille, $maven_key, $mode_collecte, $utilisateur_collecte): array
    {
        ...
        if ($mesure['code'] === 200){
            ...
            $collecte[] = [
                    ...
                    <div>
                        <ul class="open-sans">
                            <li class="lead">Clé maven : ' . $item['maven_key'] . '</li>
                            <li class="lead">Version du projet : ' . $item['project_name'] . '</li>
                            <li class="lead">Nombre de ligne : ' . $item['lines'] . '</li>
                            <li class="lead">Nombre de ligne de code : ' . $item['ncloc'] .'</li>
                            <li class="lead">Nombre de classes : ' . $item['classes'] .'</li>
                            <li class="lead">Nombre de fonctions/méthodes : ' . $item['functions'] .'</li>
                            <li class="lead">Nombre de fichiers : ' . $item['files'] .'</li>
                            <li class="lead">Distribution des langages : ' . $item['language_distribution'] .'</li>
                            <li class="lead">Ratio de dette technique : ' . $item['sqale_debt_ratio'] .'</li>
                            <li class="lead">Couverture du code : ' . $item['coverage'] .'</li>
                            <li class="lead">Codes dupliqué : ' . $item['duplicated'] .'</li>
        ...
        if ($hotspot['code'] === 200){
            ...
            /** On met à jour le tableau des données pour la table historique */
            $mergeHistorique = array_merge($mergeHistorique, $hotspot['historique']);
        } else {
        ...
        $noteHotspot = $this->batchCollecteNote->batchCollecteNoteHotspot(
            ...
            if ($noteHotspot['code']===200){
                ...
                $collecte[] = [
                        ...
                        <div>
                            <ul class="open-sans">
                                <li class="lead">Note pour les menaces potentielles : ' . $item['historique']['note_hotspot'] . '</li>
                            </ul>
                        </div>'
                    ];

                /** On met à jour le tableau des données pour la table historique */
                $mergeHistorique = array_merge($mergeHistorique, $noteHotspot['historique']['note_hotspot']);
            } else {
        ...
        if ($hotspotDetails['code'] === 200){
            $collecte[] = [
                    ...
                    <div>
                        <ul class="open-sans">
                            <li class="lead">Nombre d’élément dans la liste  : ' . $hotspotDetails['nombre'] ?? "Je ne sais pas." . '</li>
                        </ul>
                    </div>'
                ];
        ...
        foreach ($owaspKeys as $owaspKey) {
            ...
            if ($hotspotOwasp['code'] === 200) {
                if (isset($hotspotOwasp['data']) && !empty($hotspotOwasp['data'])){
                    $html = $this->generateHtmlForItem($hotspotOwasp['data']);
                    $collecte[] = [
