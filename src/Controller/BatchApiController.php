...
class BatchApiController extends AbstractController
{
  ...
  public function BatchAnomalie($mavenKey): array
  {
    ...
    if ($result1["paging"]["total"] != 0) {
      ...
      /** modules */
      $frontend=$backend=$autre=$nombreAnomalie=$erreur=0;
   ...
   * @author     Laurent HADJADJ <laurent_h@me.com>
   */
  public function batchNouvelleCollecte($mavenKey): array
  {
     ...
     */
    $batchInformation=$this->batchInformation($mavenKey);
     ...
     *  "issues" => 2677
     */
    $batchMesure=$this->batchMesure($mavenKey);

    /**
     * "note" => array:1
     *  "value" => 3 --> [A, B, C , D , E, F]
     * */
    $batchNoteReliability=$this->batchNote($mavenKey, "reliability");
    $batchNoteSecurity=$this->batchNote($mavenKey, "security");
    $batchNoteSqale=$this->batchNote($mavenKey, "sqale");
     ...
     *    "no_sonar" => 0
     */
    $batchNoSonar=$this->batchNoSonar($mavenKey);
    ...
    /**
      ...
      *   "code_smell" => 2664
      */
    $batchAnomalie=$this->batchAnomalie($mavenKey);
    /**
     ...
     *  "code_smell_info" => 325
     */
    $batchAnomalieDetails=$this->batchAnomalieDetails($mavenKey);
