...
# ==============================================================================
# Usage :
#   .\bin\e2e\rebuild-database.ps1                    # défaut: localhost/postgres
...
if ($exit -ne 0) {
    Write-Host ""
    Write-Host "[ÉCHEC] Exit code $exit" -ForegroundColor Red
    exit $exit
}

Write-Host ""
Write-Host "[OK] Rebuild termine. Base ma_moulinette prête." -ForegroundColor Green
