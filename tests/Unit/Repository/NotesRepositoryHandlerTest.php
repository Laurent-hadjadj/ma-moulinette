...
/**
 * [Description NotesRepositoryHandlerTest]
 */
class NotesRepositoryHandlerTest extends TestCase
{
    private static $mavenKey = 'fr.ma-petite-entreprise:ma-moulinette';
    private static $gestionTest = 'gestion test';

    public function testSelectNotesMavenType_WhenSQLException(): void
    {
        ...
        $repo = $this->getMockBuilder(NotesRepository::class)
        ...
        $map = ['maven_key' => static::$mavenKey, 'type' => 'reliability'];
        $result = $repo->selectNotesMavenType($map);

        // 9) Vérification
        $this->assertSame($expected, $result);
    }

    public function testInsertNotes_WhenSQLException(): void
    {
        ...
        $map = ['maven_key' => static::$mavenKey, 'type' => 'reliability',
                'value'=>3, 'mode_collecte' => 'TRAITEMENT MANUEL','utilisateur_collecte' => 'laurent.hadjadj@ma-petite-entreprise.fr',
        ...
        $result = $repo->insertNotes($map);

        $this->assertSame($expected, $result);
    }

    public function testDeleteNotesMavenKey_WhenSQLException(): void
    {
        // 1) Crée une vraie exception qui implémente DBAL\Exception et Throwable
        ...
        // 9) Appel de la méthode : elle doit entrer dans le catch et renvoyer $expected
        $result = $repo->deleteNotesMavenKey($map);
